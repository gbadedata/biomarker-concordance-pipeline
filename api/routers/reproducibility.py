...
async def create_reproducibility_result(payload: ReproducibilityResultCreate, db: AsyncSession = Depends(get_db)):
...
async def list_reproducibility_results(
    sample_id: str | None = None, passing_only: bool = False,
    ...
    if passing_only: q = q.where(ReproducibilityResult.overall_pass)
...
async def get_latest(sample_id: str, db: AsyncSession = Depends(get_db)):
    row = (await db.execute(
        select(ReproducibilityResult)
        .where(ReproducibilityResult.sample_id == sample_id)
        .order_by(ReproducibilityResult.created_at.desc()).limit(1)
    )).scalar_one_or_none()
    if not row: raise HTTPException(404, f"No reproducibility data for '{sample_id}'.")
    return row
