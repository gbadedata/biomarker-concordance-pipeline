...
async def create_reproducibility_result(
    payload: ReproducibilityResultCreate,
...
async def list_reproducibility_results(
    ...
    if sample_id:
        q = q.where(ReproducibilityResult.sample_id == sample_id)
    if passing_only:
        q = q.where(ReproducibilityResult.overall_pass)
