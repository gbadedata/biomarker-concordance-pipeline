...
async def create_concordance_result(payload: ConcordanceResultCreate, db: AsyncSession = Depends(get_db)):
...
async def list_concordance_results(
    variant_type: str | None = None, passing_only: bool = False,
    ...
    if passing_only: q = q.where(ConcordanceResult.f1_pass)
...
async def concordance_summary(sample_id: str, db: AsyncSession = Depends(get_db)):
    q = (
        select(ConcordanceResult.variant_type,
    ...
    if not rows: raise HTTPException(404, f"No concordance data for '{sample_id}'.")
