...
async def list_alerts(
    unresolved_only: bool = True, severity: str | None = None,
    limit: Annotated[int, Query(ge=1, le=500)] = 100,
    offset: Annotated[int, Query(ge=0)] = 0,
    db: AsyncSession = Depends(get_db),
):
    q = select(QualityAlert).order_by(QualityAlert.created_at.desc())
    if unresolved_only: q = q.where(not QualityAlert.resolved)
    if severity: q = q.where(QualityAlert.severity == severity)
...
async def resolve_alert(alert_id: str, db: AsyncSession = Depends(get_db)):
    alert = (await db.execute(select(QualityAlert).where(QualityAlert.id == uuid.UUID(alert_id)))).scalar_one_or_none()
