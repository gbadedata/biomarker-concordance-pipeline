...
async def create_run(payload: PipelineRunCreate, db: AsyncSession = Depends(lambda: get_db())):
...
async def get_run(run_id: str, db: AsyncSession = Depends(lambda: get_db())):
...
async def update_run(run_id: str, payload: PipelineRunUpdate, db: AsyncSession = Depends(lambda: get_db())):
    run = (await db.execute(select(PipelineRun).where(PipelineRun.run_id == run_id))).scalar_one_or_none()
    if not run:
        raise HTTPException(404, f"Run '{run_id}' not found.")
